...
webpackConfig.devServer = (devServerConfig) => {
  // Migrate deprecated webpack-dev-server v4 API to v5 schema
  if (devServerConfig.onBeforeSetupMiddleware || devServerConfig.onAfterSetupMiddleware) {
    ...
    devServerConfig.setupMiddlewares = (middlewares, devServer) => {
      ...
      const mws = existing ? existing(middlewares, devServer) : middlewares;
      if (afterMw) afterMw(devServer);
      return mws;
    };
  }
  // Migrate `https` to `server` for v5
  if (Object.prototype.hasOwnProperty.call(devServerConfig, 'https')) {
    const httpsVal = devServerConfig.https;
    delete devServerConfig.https;
    if (httpsVal === true) devServerConfig.server = 'https';
    else if (httpsVal && typeof httpsVal === 'object') devServerConfig.server = { type: 'https', options: httpsVal };
  }
  // Remove other removed properties
  ['http2'].forEach((k) => { if (k in devServerConfig) delete devServerConfig[k]; });
